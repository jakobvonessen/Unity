...
* `_.dPos(i, steps, origPos, diffPos)` uses _.dSine() to return a Vector3 representing a point in space between (*origPos*) and (*origPos + diffPos*), that is `_.dSine(i, steps)` of the way along the two. Used for moving a *gameObject*'s *transform* from one point to another during *steps* frames in a smooth fashion

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkwMjM3NTE5NiwxMTQyNTk5NDg1LC0xOD
